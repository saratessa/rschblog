...
icc_values <- data.frame(
  Model = c("Social Skills", "Problem Behavior"),
  ICC = c(icc_social, icc_behavior)
)

ggplot(icc_values, aes(x = Model, y = ICC, fill = Model)) +
  geom_bar(stat = "identity") +
  labs(title = "Intraclass Correlation Coefficient (ICC)",
       y = "Proportion of Variance Explained by Schools",
       x = "Outcome Variable") +
  theme_minimal()
```
...
```{r}
#Scatterplot of Vocabulary vs. Social Skills
ggplot(data_long, aes(x = ppvtst_c, y = ssisss_c)) +
  geom_point(alpha = 0.5, color = "blue") +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Relationship Between Vocabulary and Social Skills",
       x = "Centered Vocabulary Score",
       y = "Centered Social Skills Score") +
  theme_minimal()
```
...
knitr::include_url("HLM_Results.html")
```
